...
const LoadingSpinner = ({ size = 'md', text = '' }) => {
  ...
  return (
    <div className="flex flex-col items-center justify-center gap-3">
      <div className={`${sizes[size]} border-surface-300 border-t-brand-500 rounded-full animate-spin`} />
      {text && <span className="text-xs font-medium text-surface-400 tracking-wide">{text}</span>}
    </div>
  );
};
...
export const PageLoader = ({ text = 'Loading...' }) => (
  <div className="min-h-[60vh] flex flex-col items-center justify-center gap-4">
    <div className="relative">
      <div className="w-12 h-12 border-2 border-surface-200 border-t-brand-500 rounded-full animate-spin" />
