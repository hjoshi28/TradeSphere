  ...
  return (
    <div className="max-w-7xl mx-auto space-y-6">
      <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }}>
        <h1 className="text-2xl sm:text-3xl font-bold text-surface-900 mb-1">Market</h1>
      ...
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }}
        ...
        <HiMagnifyingGlass className="absolute left-4 top-1/2 -translate-y-1/2 w-4 h-4 text-surface-400" />
          ...
          <div className="absolute top-full left-0 right-0 mt-2 glass rounded-xl overflow-hidden shadow-2xl z-10">
            {searchResults.map((r, i) => (
              <Link key={i} to={`/market/${r.symbol}`} onClick={() => { setSearchQuery(''); setSearchResults([]); }}
                className="flex items-center justify-between px-4 py-3 hover:bg-surface-100/40 transition-colors">
                <div>
                  <span className="text-sm font-bold text-surface-900">{r.symbol}</span>
      ...
      {trending.length > 0 && (
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.15 }}>
          <h2 className="text-lg font-bold text-surface-900 mb-4">Trending Stocks</h2>
              ...
              return (
                <Link key={i} to={`/market/${stock.symbol}`} className="card card-hover p-4">
                  <div className="flex items-center justify-between mb-2">
                    <span className="font-bold text-surface-900 font-mono">{stock.symbol}</span>
                    <Badge variant={isUp ? 'gain' : 'loss'} size="xs">{isUp ? '+' : ''}{stock.percentChange?.toFixed(2)}%</Badge>
                  </div>
                  <span className="text-lg font-bold font-mono text-surface-800">${stock.price?.toFixed(2)}</span>
                </Link>
              );
            })}
          </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }}
          ...
          <div className="px-5 py-4 border-b border-surface-200/60 flex items-center gap-2">
            <HiOutlineArrowTrendingUp className="w-4 h-4 text-gain" />
            <h2 className="text-sm font-bold text-surface-800">Top Gainers</h2>
          </div>
          <div className="divide-y divide-surface-200/40">
            {(movers.gainers || []).map((s, i) => (
              <Link key={i} to={`/market/${s.symbol}`}
                className="flex items-center justify-between px-5 py-3.5 hover:bg-surface-100/20 transition-colors">
                <div className="flex items-center gap-3">
                  <span className="font-bold text-surface-900 font-mono text-sm">{s.symbol}</span>
                </div>
                <div className="flex items-center gap-3">
                  <span className="font-mono text-surface-700 text-sm">${s.price?.toFixed(2)}</span>
                  <Badge variant="gain" size="xs">+{s.percentChange?.toFixed(2)}%</Badge>
                </div>
              </Link>
            ))}
            {(!movers.gainers || movers.gainers.length === 0) && (
              <p className="px-5 py-8 text-sm text-surface-400 text-center">No gainer data available</p>
            )}
          </div>
        </motion.div>
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.25 }}
          ...
          <div className="px-5 py-4 border-b border-surface-200/60 flex items-center gap-2">
            ...
            <h2 className="text-sm font-bold text-surface-800">Top Losers</h2>
          </div>
          <div className="divide-y divide-surface-200/40">
            ...
            {(!movers.losers || movers.losers.length === 0) && (
              <p className="px-5 py-8 text-sm text-surface-400 text-center">No loser data available</p>
            )}
          </div>
        </motion.div>
      </div>
      ...
      {news.length > 0 && (
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.3 }}>
          <h2 className="text-lg font-bold text-surface-900 mb-4">Market News</h2>
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
            {news.map((n, i) => (
              <a key={i} href={n.url} target="_blank" rel="noopener noreferrer" className="card card-hover p-4 group">
                ...
                <h3 className="text-sm font-semibold text-surface-800 line-clamp-2 mb-2 group-hover:text-white transition-colors">{n.headline}</h3>
                <p className="text-xs text-surface-400">{n.source} · {n.datetime ? new Date(n.datetime * 1000).toLocaleDateString() : ''}</p>
              </a>
            ))}
          </div>
        </motion.div>
